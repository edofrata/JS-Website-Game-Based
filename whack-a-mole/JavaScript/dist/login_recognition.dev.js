...
function checkLogin() {
  if (sessionStorage.logged_in_user !== undefined) {
    ...
    var user = JSON.parse(localStorage[sessionStorage.logged_in_user]); //Say hello to logged in user
...
function login_real() {
  ...
  document.getElementById("login_username").style.backgroundColor = "#FFFFFF"; // retriving the usernzmd inserted 
  ...
  if (localStorage[username] === undefined) {
    ...
    var _user_login = JSON.parse(localStorage[username]); //COnvert to JS
    // retriving password


    var password = document.getElementById("login_password").value; // checking ig the password is in the system

    if (password === _user_login.password) {
      // clears any failures
      document.getElementById("login_failure").innerHTML = ""; // we save the session in loggedInUser

      sessionStorage.logged_in_user = _user_login.username; // Relocates the page if everything went through
